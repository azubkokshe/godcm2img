...
void *DCM2PNM::convert(int *outSize, const char *opt_ifname, unsigned int opt_frame, int outputFormat) {
    ...
    if (!opt_suppressOutput) {
        ...
        unsigned long dSize = di->getOutputDataSize();
        stream = open_memstream(&bResult, &dSize);
        ...
        *outSize = dSize;

        {
            ...
            {
                case EFT_RawPNM:
                    result = di->writeRawPPM(stream, 8, opt_frame);
                    break;
                case EFT_8bitPNM:
                    result = di->writePPM(stream, 8, opt_frame);
                    break;
                case EFT_16bitPNM:
                    result = di->writePPM(stream, 16, opt_frame);
                    break;
                case EFT_NbitPNM:
                    result = di->writePPM(stream, OFstatic_cast(int, opt_fileBits), opt_frame);
                    break;
                case EFT_BMP:
                    result = di->writeBMP(stream, 0, opt_frame);
                    break;
                case EFT_8bitBMP:
                    result = di->writeBMP(stream, 8, opt_frame);
                    break;
                case EFT_24bitBMP:
                    result = di->writeBMP(stream, 24, opt_frame);
                    break;
                case EFT_32bitBMP:
                    result = di->writeBMP(stream, 32, opt_frame);
                    break;
                case EFT_JPEG:{
                        DiJPEGPlugin plugin;
                        plugin.setQuality(OFstatic_cast(unsigned int, opt_quality));
                        plugin.setSampling(opt_sampling);
                        result = di->writePluginFormat(&plugin, stream, opt_frame);
                    }
                    break;
                case EFT_PNG: {
                        ...
                        result = di->writePluginFormat(&pngPlugin, stream, opt_frame);
                    }
                    break;
                default:
                      result = di->writePPM(stream, 8, opt_frame);
